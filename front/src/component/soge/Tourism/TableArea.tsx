...
import TourismCustomerTable from "@/component/soge/Tourism/CustomerTable";
import VisitingCustomers from "@/component/soge/VisitingCustomers";
import Overlay from "@/component/soge/Index/Overlay";
import putUpdateTime from "@/component/soge/Requests/Tourism/putUpdateTime";
import postNewData from "@/component/soge/Requests/Tourism/postNewData";
...
const TourismTableArea = ({ date, setDate, shareKey, onSubmit }: Props) => {
  ...
  const fetchVisitings = async () => {
    ...
    setCanShareData(res.data.can_share_data || false);
  };
  ...
  // 送迎タイプのチェック関数（エラー通知も含む）
  const checkSogeTypeCompatibility = (
    activeUser: Customer | undefined,
    overBin: TourismVisitingGroup | undefined
  ): boolean => {
    if (!activeUser || !overBin) return true;

    // 便内に利用者がいない場合は許可
    if (!overBin.customers || overBin.customers.length === 0) {
      return true;
    }

    // 便内の最初の利用者の送迎タイプを取得
    const existingSogeType = overBin.type;
    const activeSogeType = activeUser.soge_type;

    // 送迎タイプが異なる場合はエラー通知してfalseを返す
    if (
      existingSogeType &&
      activeSogeType &&
      existingSogeType !== activeSogeType
    ) {
      const sogeTypeLabel = existingSogeType === "pick_up" ? "迎え" : "送り";
      ErrorToast(
        `この便は${sogeTypeLabel}専用です。異なる送迎タイプの利用者は追加できません。`
      );
      return false;
    }
  ...
  const moveCustomerToVisiting = async (
    ...
    overBin: TourismVisitingGroup | undefined,
    activeUser: Customer | undefined,
    overUser: Customer | undefined
  ) => {
    let newOverBinRoutePoints: TourismRoutePoint[] = [];
    let newOverBinCustomers: Customer[] = [];

    if (activeUser && overBin) {
      // 送迎タイプのチェック
      if (!checkSogeTypeCompatibility(activeUser, overBin)) {
        resetActiveId();
        return;
      }
      ...
      await new Promise((resolve) =>
        setVisitingsGroups((prev) => {
          const newBins = [...prev];
          // 対象の便に利用者を追加
          const routePoint: TourismRoutePoint = {
            ...activeUser,
            visiting_id: Number(overBin.id),
          };
          overBin.route_points.push(routePoint);
          overBin.customers.push(activeUser);
          ...
          newOverBinCustomers = overBin.customers;
          resolve(newBins);
          return newBins;
        })
      );
    ...
    if (activeUser && !overUser && !overBin) {
      ...
      await new Promise((resolve) =>
        setVisitingsGroups((prev) => {
          ...
          newOverBinCustomers = [activeUser];
          resolve(newBins);
          return newBins;
        })
      );
  ...
  const handleDragEnd = async (event: DragOverEvent) => {
    ...
    let newActiveBinRoutePoints: TourismRoutePoint[] = [];
    let newOverBinRoutePoints: TourismRoutePoint[] = [];
    let newActiveBinCustomers: Customer[] = [];
    let newOverBinCustomers: Customer[] = [];
    // パターン1 既存の便内で並び替え
    if (
      activeUser &&
      ...
      await new Promise((resolve) =>
        setVisitingsGroups((prev) => {
          ...
          newActiveBinCustomers = activeBin.customers;
          resolve(newBins);
          return newBins;
        })
      );
      ...
      await putUpdateTime(
        activeBin.id,
        newActiveBinRoutePoints,
        newActiveBinCustomers
      );
      fetchVisitings();
      return;
    }
    ...
    if (activeUser && activeBin && overBin && activeBin !== overBin) {
      // 送迎タイプのチェック
      if (!checkSogeTypeCompatibility(activeUser, overBin)) {
        resetActiveId();
        return;
      ...
      await new Promise((resolve) =>
        setVisitingsGroups((prev) => {
          ...
          activeBin.customers = activeBin.customers.filter(
          ...
          newActiveBinRoutePoints = activeBin.customers;
          ...
          newBins[overRowIndex][overColIndex] = overBin;
          newOverBinRoutePoints = overBin.customers;
          resolve(newBins);
    ...
    if (activeUser && !overUser && activeBin && !overBin) {
      ...
      await new Promise((resolve) =>
        setVisitingsGroups((prev) => {
          ...
          newActiveBinCustomers = activeBin.customers.filter(
            (customer) => customer.id !== activeUser.id
          );
          newOverBinCustomers = [activeUser];
          resolve(newBins);
          return newBins;
  ...
  const handleShareToHotels = useCallback(async () => {
    const url = "/tourism/visitings/share_to_office";
              ...
              <div className="panelSCT">
                <div className="cont">
                  <div className="head" style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <div className="selectbox">
                      {onSubmit && (
                        <button type="button" aria-label="登録" onClick={onSubmit}>
...
export default TourismTableArea;
