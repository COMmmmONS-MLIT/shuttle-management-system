...
import BlankArea from "../Index/BlankArea";
...
const TourismBinBlock = ({
  ...
  const handleDeleteRoutePoint = async (routePointId: number) => {
    const customerToDelete = bin.customers.find(
      (customer) => customer.id === routePointId
    );
    ...
    await handleDelete(
      `/visitings_customers/${customerToDelete.id}/remove_from_visiting`
    );
      if (bin.id && bin.customers.length > 0) {
        const remainingCustomers = bin.customers.filter(
          (customer) => customer.id !== routePointId
        );

        const hasVisitingsCustomer = remainingCustomers.some(
          (customer) => customer.point_type === "VisitingsCustomer"
        );

        if (hasVisitingsCustomer) {
          await putUpdateTime(bin.id, bin.route_points, remainingCustomers);
        }
      }
      fetchVisitings();
  };
        ...
        <td ref={setNodeRef} style={{
          ...
          >
            ...
            <ul className="user js-user" style={{
              ...
              {(() => {
                const visitingsPoints = bin.route_points.filter(
                  (rp) => rp.point_type === "VisitingsPoint"
                );
                return bin.route_points.map((routePoint) => {
                  const visitingsPointIndex = visitingsPoints.findIndex(
                    (rp) => rp.id === routePoint.id
                  );
                  const orderNumber =
                    visitingsPointIndex !== -1 ? visitingsPointIndex + 1 : undefined;
                  const isLast =
                    routePoint.point_type === "VisitingsPoint" &&
                    visitingsPointIndex === visitingsPoints.length - 1;
...
export default TourismBinBlock;
