with visiting_counts as (
  select 
    ...
    sum(vc_count.passenger_count) as total_count,
...
customer.name,
customer.name_kana,
case when vc.soge_type = 1 then office_bookmark.bid else customer_bookmark.bid end as pickup_point_bid,
vc.soge_type,
vc.schedule_time as schedule_time,
vc.start_time as start_time,
case when vc.soge_type = 1 then concat(ifnull(office_bookmark.住所,''),ifnull(office_bookmark.号室名,'')) 
     else concat(ifnull(customer_bookmark.住所,''),ifnull(customer_bookmark.号室名,'')) end as pickup_address,
case when vc.soge_type = 1 then customer_bookmark.bid else office_bookmark.bid end as office_point_bid,
case when vc.soge_type = 1 then concat(ifnull(customer_bookmark.住所,''),ifnull(customer_bookmark.号室名,'')) 
     else concat(ifnull(office_bookmark.住所,''),ifnull(office_bookmark.号室名,'')) end as dropoff_address,
case when vc.soge_type = 1 then office_bookmark.lat else customer_bookmark.lat end as pickup_lat,
case when vc.soge_type = 1 then office_bookmark.lng else customer_bookmark.lng end as pickup_lng,
case when vc.soge_type = 1 then office_bookmark.停車lat else customer_bookmark.停車lat end as pickup_stop_lat,
case when vc.soge_type = 1 then office_bookmark.停車lng else customer_bookmark.停車lng end as pickup_stop_lng,
case when vc.soge_type = 1 then customer_bookmark.lat else office_bookmark.lat end as office_lat,
case when vc.soge_type = 1 then customer_bookmark.lng else office_bookmark.lng end as office_lng,
concat(
    if((vc.soge_type mod 1)>0,if(p_bookmark.地点<>'A',concat(customer_bookmark.住所ラベル,'への迎え '),'')
...
case when vc.soge_type = 1 then office_bookmark.車両制限 else customer_bookmark.車両制限 end as pickup_car_restriction,
customer.wc,
customer.walker_size,
vc.is_absent,
vc.absence_reason,
customer.walker,
case when vc.soge_type = 1 then office_bookmark.住所ラベル else customer_bookmark.住所ラベル end as pickup_address_label,
case when vc.soge_type = 1 then customer_bookmark.住所ラベル else office_bookmark.住所ラベル end as dropoff_address_label,
null as reserved1,
car_pattern.name as car_pattern_name,
car.max_seat,
car.max_wc_seat,
coalesce(customer.wc, 0) as wheelchair_count,
case when vc.soge_type = 1 then office_bookmark.待ち時間 else customer_bookmark.待ち時間 end as wait_time,
...
left join bookmark office_bookmark on office.cd=office_bookmark.事業所cd and office_bookmark.参考id='A001'
