...
json.customer do
  ...
  if @customer.user.present?
    json.user do
      json.extract! @customer.user, :email
    end
  end
end
...
json.use_cases @customer.use_cases do |use_case|
  json.extract! use_case, :id, :customer_id, :customer_id, :day_of_week,
  ...
  if use_case.pick_up_point_id.present?
    json.pick_up_point_order bookmarks_with_order.find { |b|
                               b[:bookmark].bid == use_case.pick_up_point_id
                             }[:order]
  end
  if use_case.drop_off_point_id.present?
    json.drop_off_point_order bookmarks_with_order.find { |b|
                                b[:bookmark].bid == use_case.drop_off_point_id
                              }[:order]
  end
  json.departure_time use_case.formatted_departure_time if use_case.departure_time.present?
  json.arrival_time use_case.formatted_arrival_time if use_case.arrival_time.present?
  json.start_time use_case.formatted_start_time if use_case.start_time.present?
end
